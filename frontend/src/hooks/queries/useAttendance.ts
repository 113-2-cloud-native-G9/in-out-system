...
export const attendanceKeys = {
  ...
  employeeRecords: (employeeId: string, month?: string) => [...attendanceKeys.records(), employeeId, month] as const,
};
...
export const usePersonalAttendanceRecords = (month?: string) => {
  ...
  return useQuery({
    queryKey: attendanceKeys.employeeRecords(user?.employee_id || '', month),
    queryFn: () => attendanceApi.getPersonalAttendance(user!.employee_id, month),
...
// 獲取特定員工的考勤記錄
export const useEmployeeAttendanceRecords = (employeeId: string, month?: string) => {
  return useQuery({
