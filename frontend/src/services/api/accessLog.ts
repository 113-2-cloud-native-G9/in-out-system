...
export const accessLogApi = {
    ...
    getPersonalAccessLogs: async (
        date: string
    ): Promise<{ logs: AccessLog[] }> => {
        return (
            (await fetchWithJwt<{ logs: AccessLog[] }>(
                `${BASE_API}/access-logs?date=${date}`,
                HttpMethod.GET
            )) || []
        );
    },
    ...
    getEmployeeAccessLogsByDate: async (
        employeeId: string,
        date: string
    ): Promise<AccessLog[]> => {
        return (
            (await fetchWithJwt<AccessLog[]>(
    ...
    getEmployeeAccessLogs: async (employeeId: string): Promise<AccessLog[]> => {
        return (
            (await fetchWithJwt<AccessLog[]>(
                `${BASE_API}/access-logs/employees/${employeeId}`,
                HttpMethod.GET
            )) || []
        );
    },
